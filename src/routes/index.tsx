...
import { carousels, blogs, newsletters, meta } from "@/lib/paywise-content";

export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "PayWise — Content Preview" },
      {
        name: "description",
        content:
          "Monthly content drop preview for PayWise: Instagram grid, carousels, blog posts and newsletters.",
      },
    ],
  }),
  component: Index,
      ...
      >
        {meta.gridPreview ? (
          <button
            ...
            onClick={() =>
              setLightbox({
                src: meta.gridPreview,
                alt: "IG grid preview",
                aspect: "square",
                downloadFilename: "paywise-grid-preview.png",
              })
            }
            className="mx-auto block max-w-md overflow-hidden rounded-lg border border-border bg-card group relative"
          >
            <img src={meta.gridPreview} alt="Instagram grid preview" className="w-full" />
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation();
                e.preventDefault();
                downloadImage(meta.gridPreview, "paywise-grid-preview.png");
              }}
              ...
              aria-label="Download grid preview"
            >
              <Download className="h-3.5 w-3.5" />
            </button>
          </button>
        ) : (
          <div className="mx-auto grid max-w-md grid-cols-3 gap-1 sm:gap-2">
            {carousels.map((c) => {
